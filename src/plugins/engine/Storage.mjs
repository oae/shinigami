...
export default class Storage {
    ...
    async saveConfig(key, value, indentation) {
        return new Promise((resolve, reject) => {
            fs.writeFile(this.config + key, JSON.stringify(value, undefined, indentation), function (error) {
                if (error) {
                    reject(error);
                } else {
                    resolve();
                ...
                let media = {
                    ...
                    subtitles: subtitles.sort().map(subtitle => {
                        ...
                        return {
                            ...
                            content: fs.readFileSync(path.join(directory, subtitle), { encoding: 'utf-8' })
                        };
                    })
                };
    ...
    _makeValidFileURL(directory, file) {
        return encodeURI('file://' + path.join(directory, file).replace(/\\/g, '/'))
    ...
    _saveChapterPagesEPUB(ebook, pageData) {
        ...
        let title = `${path.basename(path.dirname(ebook))} ${path.sep} ${path.basename(ebook, extensions.epub)}`;
    ...
    concatVideoChunks(chapter, files, index, fileOut) {
        return new Promise((resolve, reject) => {
            ...
            if (!fileOut) {
                ...
                let file = path.join(directory, this.sanatizePath(chapter.title + extensions.mp4));
    ...
    saveChapterFileM3U8(chapter, content) {
        try {
            ...
            file = path.join(file, this.sanatizePath(chapter.title + extensions.m3u8));
    ...
    muxPlaylistM3U8(chapter, ffmpeg) {
        return new Promise((resolve, reject) => {
            let directory = this._mangaOutputPath(chapter.manga);
            this._createDirectoryChain(directory);
            let file = path.join(directory, this.sanatizePath(chapter.title + extensions.mkv));
            directory = path.join(directory, this.sanatizePath(chapter.title + extensions.m3u8));
    ...
    saveBookmarks(key, value, indentation) {
        return new Promise((resolve, reject) => {
            fs.writeFile(this._bookmarkOutputPath + key, JSON.stringify(value, undefined, indentation), function (error) {
                if (error) {
                    reject(error);
                } else {
                    resolve();
