...
export default class Request {
    ...
    async _checkScrapingRedirection(page) {
        let scrapeRedirect = await page.evaluate(this._scrapingCheckScript);
    ...
    _extractRequestOptions(request) {
        ...
        return {
            ...
            httpReferrer: referer ? referer : request.url,
    ...
    async fetchUI(request, injectionScript, timeout, images) {
        Logger.log('Fetching UI for', request.url);
        ...
        return new Promise(async (resolve, reject) => {
            ...
            page.on('request', (request) => {
                ...
                if (Engine.Blacklist.patterns.some(pattern => new RegExp(pattern.replace(/\*/g, '.*')).test(request.url))) {
                    request.abort();
                } else {
                    request.continue();
                }
            ...
            page.on('domcontentloaded', () => page.evaluate(this._domPreparationScript));

            page.on('load', async () => {
                try {
                    if (await this._checkScrapingRedirection(page)) {
                        return;
                    }
            ...
            const reqOpts = this._extractRequestOptions(request);
            page.setExtraHTTPHeaders({...reqOpts.extraHeaders, 'referer': reqOpts.httpReferrer, 'user-agent': reqOpts.userAgent});
